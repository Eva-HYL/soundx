  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Row gutter={16}>
        ...
        <Col span={12}>
          <Card
            title={<span>今日待办 <Text type="secondary" style={{ fontSize: 12, fontWeight: 400 }}>共 28 项</Text></span>}
            size="small"
          >
        ...
        <Col span={12}>
          <Card title="订单状态分布 · 本日" size="small" style={{ marginBottom: 16 }}>
            <Space direction="vertical" style={{ width: '100%' }} size={8}>
              {ORDER_DIST.map((r) => (
                <div key={r.status} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
                  <Tag color={r.color} style={{ minWidth: 60, textAlign: 'center', margin: 0 }}>{r.status}</Tag>
                  <Progress
                    percent={r.pct}
                    showInfo={false}
                    strokeColor={r.color}
                  ...
                  />
                  <Text style={{ width: 32, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{r.count}</Text>
                </div>
              ))}
            </Space>
          </Card>
          ...
          <Card title="本周 Top 陪玩" size="small">
            {TOP_PALS.map((p, i) => (
              <div key={p.name} style={{ display: 'flex', alignItems: 'center', padding: '6px 0', fontSize: 13 }}>
                <Text type="secondary" style={{ width: 20 }}>{i + 1}</Text>
                <Text style={{ flex: 1 }}>{p.name}</Text>
                <Text type="secondary" style={{ width: 50, textAlign: 'right' }}>{p.orders} 单</Text>
                <Text strong style={{ width: 80, textAlign: 'right' }}>{p.revenue}</Text>
              </div>
            ))}
          </Card>
        </Col>
